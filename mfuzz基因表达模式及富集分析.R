...
rm(list=ls())
getwd()
setwd("D:/anxiiuli_data/count_炎红/count_yr")


cts<-read.csv("ternimalE_fpkm.csv",row.names = 1)
cts<-cts[which(rowSums(cts) > 10),]#去掉全为零的行 情况
...
normal$day0<-apply(normal[,1:2],1, mean, na.rm = T) 
normal$day05<-apply(normal[,3:4],1, mean, na.rm = T) 
normal$day15<-apply(normal[,5:6],1, mean, na.rm = T) 

normal <-normal[,7:9]
normal <- data.matrix(normal)
dir.create("./mfuzz_fpkm10")
setwd("./mfuzz_fpkm10")
...
## 绘制折线图
pdf("mfuzz.pdf",width=10,height=10)
...
fpkm<-read.csv(file="D:/anxiiuli_data/count_炎红/count_yr/ternimalE_fpkm.csv",row.names = 1)
head(fpkm)
